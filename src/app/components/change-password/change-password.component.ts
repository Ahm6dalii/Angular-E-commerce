...
export class ChangePasswordComponent {
  ...
  constructor(private messageService:MessageService,private _userSettingService:UserSettingService){
    ...
    currentPassword: new FormControl('', [Validators.pattern(new RegExp('^[0-9]{6,12}$'))]),
    password: new FormControl('', [Validators.pattern(new RegExp('^[0-9]{6,12}$'))]),
...
get passwordControl(): FormControl {
  ...
  updateUserInformation(event: Event) {
    ...
    if (this.updateForm.valid == false) {
      ...
      this._userSettingService.updateUserPassword(this.updateForm.value).subscribe({
        next: (res) => {
